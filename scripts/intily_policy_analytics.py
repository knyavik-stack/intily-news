...
def print_weight_policy():
    ...
    print('- Случайный бонус региона не используется. География не изменяет математический score и не должна переопределять final-score ordering.')
    print('- После AI-оценки очередь и публикация сортируются по итоговому score по убыванию; время используется только как tie-breaker.')
    print('\n### Категории\n')
    print('| Вес | Категория |')
    print('|---:|---|')
