...
def print_rejection(run):
    ...
    print(f'| Отсечено по весу | {score_filtered} | {pct(score_filtered, raw)} | итоговый вес ниже 50.0 |')
...
def print_weight_policy():
    ...
    print('|---|---:|---|')
    print('| AI-релевантность | 30 | прямое отношение к ИИ, LLM, нейросетям, агентам, роботам и AI-инфраструктуре |')
    print('| Влияние события | 20 | запуск, новая модель, прорыв, инвестиции, безопасность, закон, крупная сделка и другие признаки значимого события |')
    print('| Практическая ценность | 15 | внедрение, автоматизация, разработка, инструменты, инфраструктура, бизнес-кейсы и применимость |')
    print('| Новизна/конкретность | 10 | эксклюзивность, «впервые», крупные/рекордные события и числовая конкретика |')
    print('| Качество источника | 10 | приоритетный источник 10; доверенный 7; прочий 4 |')
    print('| Доказательность | 5 | информативность описания; длинное описание не получает линейного неограниченного бонуса |')
    print('| Свежесть | 5 | непрерывное снижение от 5.0 к 0.0 за 12 часов, без грубых ступеней |')
    ...
    print('')
    print('**Порог допуска: 50.0.** Балл ниже порога не попадает в очередь. Порог установлен после production-калибровки: при 60.0 последний запуск получил 0 из 403 материалов выше порога.')
    print('')
    print('### Почему один знак после запятой не означает искусственную уникальность')
    print('')
    print('Свежесть и доказательность считаются непрерывно, поэтому 50.1 и 50.2 имеют реальный смысл. Если две новости действительно имеют одинаковую оценку, система не добавляет случайный шум: они остаются равными по весу, а порядок разрешается детерминированными признаками.')
    print('')
    print('### Категории')
    print('')
    print('| Вес | Категория |')
    print('|---:|---|')
    print('| 0–49.9 | не проходит порог |')
    print('| 50.0–84.9 | A |')
    print('| 85.0–100 | S |')
    print('')
