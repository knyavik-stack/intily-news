...
def build_edit_prompt(x, retry=False, previous_error=''):
    ...
    if style_prompt == 1:
        ...
        prompt_template = (
            'Подготовь готовый Telegram-пост, описание ЦЕЛИКОМ на естественном русском языке как журналиста-тюремного алкаша в жестком конкретно правдивом тоне, интересно, завораживающе, чётко по факту, объективно, честно, с цитированием мата. '
            'Не делай дословный перевод: перескажи человеческим быдло языком с матом используя слова и производные от них со смысловыми накланениями: охуеть, нейрохуйня, пиздец, блядь, ебануться, ёмана рот, хуй, пизда, ебать, обучен на хуях, и производные от них '
