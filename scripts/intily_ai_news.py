...
PUBLISH_INTERVAL_SECONDS = 2 * 60  # Minimum interval between Telegram publications: 3 minutes.
...
# 1 — авторский prompt Boss: жёсткий, матерный и саркастичный.
# 2 — приличный prompt: естественный русский без мата и грубых формулировок.
...
def load_state():
    ...
    # Production KPI history is intentionally bounded. It is the operator's
    # durable audit trail for business outcomes, not a debug log.
    if not isinstance(s.get('run_history'), list):
        s['run_history'] = []
...
def candidate_quality(x):
    ...
    if x['importance'] < IMPORTANCE_THRESHOLD:
        return False
...
def build_edit_prompt(x, retry=False, previous_error=''):
    # Выбор редакционного стиля происходит только здесь.
    # Пользователь сам переключает style_prompt в начале файла.
    if style_prompt == 1:
        # Авторский prompt Boss. Не менять без его прямого указания.
        prompt_template = (
            'Подготовь готовый Telegram-пост, описание ЦЕЛИКОМ на естественном русском языке как журналиста-тюремного алкаша в жестком конкретно правдивом тоне, интересно, завораживающе, чётко по факту, объективно, честно с максимальным сарказмом и жёстким цитированием мата через каждое слово. '
            'Не делай дословный перевод: перескажи человеческим быдло языком с матом используя слова и производные от них со смысловыми накланениями: ёбаный пупок, прикинь, охуеть, нейрохуйня, пиздец, блядь, по столу, гвозди забивать, Ai-оёёй, ебануться, ёмана рот, сраньё, уебан, мудак, хуй, пизда, ебать, фуфел, сука, говномес, токен тебе в рот, пиздатокен, обучен на хуях, галлюциноебля, нейронка, хуёнка и производные от них '
        ...
        # Приличный вариант: тот же фактологический каркас без мата и грубого образа.
        prompt_template = (
            'Подготовь готовый Telegram-пост, описание ЦЕЛИКОМ на естественном русском языке журналиста в живом, уверенном и профессиональном стиле. '
            ...
            'Обязательно раскрой: что произошло, кто участники, почему это важно и какой практический ценный контекстный вывод стоит из этого сделать. Можно разбивать на абзацы. Постарайся уместиться в 700 символов. '
            'Не выдумывай факты. Названия компаний, продуктов и моделей можно оставлять в оригинальном написании.\n'
        ...
        # Не допускаем запуск с неизвестным стилем. Используйте только 1 или 2.
...
# OPERATOR CONTROL:
#   KPI_MONITORING_ENABLED = True keeps a bounded run history in state and
#   exposes the same metrics in the GitHub Actions job summary.
#   Set to False only if Boss explicitly wants monitoring disabled. This does
#   NOT disable publication itself; it only stops KPI history collection.
#   Main code/settings remain operational when monitoring is disabled.
...
def main():
    ...
    if not can_publish:
        ...
        # Preserve the 60% RU invariant: while RU is below the target share, publish WORLD only when needed to avoid falling further below target.
        if queue_counts['RUSSIA'] / max(1, len(queue)) <= RUSSIA_TARGET_SHARE:
    ...
    if published:
        business_result = 'PUBLISHED'
        business_reason = 'telegram_delivery_ok'
    elif item_failures:
        business_result = 'PUBLISH_FAILED'
        business_reason = 'all_attempted_items_failed'
    elif not can_publish:
        business_result = 'NO_PUBLISH'
        business_reason = 'publish_interval_not_reached'
    elif not candidates and not s['queue']:
        business_result = 'NO_PUBLISH'
        ...
        business_result = 'NO_PUBLISH'
        business_reason = 'cycle_completed_without_publish'
