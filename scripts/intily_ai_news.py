...
def build_edit_prompt(x, retry=False, previous_error=''):
    ...
    if style_prompt == 1:
        ...
        prompt_template = (
            ...
            'Не делай дословный перевод: перескажи человеческим быдло языком с матом используя слова и производные от них со смысловыми накланениями: ёбаный пупок, прикинь, охуеть, нейрохуйня, пиздец, блядь,по столу, гвозди забивать, Aiоёёй, ебануться, ёмана рот, сраньё, уебан, мудак, хуй, пизда, ебать, ракета, токен тебе в рот, пиздатокен, обучен на хуях, галлюциноебля, нейронка-хуёнка и производные от них '
